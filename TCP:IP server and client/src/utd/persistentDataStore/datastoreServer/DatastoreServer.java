...
public class DatastoreServer
{
	...
	private ServerCommand dispatchCommand(InputStream inputStream) throws ServerException, IOException
	{
		...
		if (cmdName == null) {
			throw new ServerException("unable to parse command from " + cmdName);
		}
		else if ("read".equalsIgnoreCase(cmdName)) {
			logger.debug("Parsing read");
			cmd = new ReadCommand();
		}
		else if ("write".equalsIgnoreCase(cmdName)) {
			logger.debug("Parsing write");
		...
		else if ("directory".equalsIgnoreCase(cmdName)) {
